...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    function accumulativeDividendOf(address _owner) public view override returns (uint256) {
        return
            magnifiedDividendPerShare.mul(balanceOf(_owner)).toInt256().add(magnifiedDividendCorrections[_owner]).toUint256() / magnitude;
    }
    ...
    function _mint(address account, uint256 value) internal override {
        ...
        magnifiedDividendCorrections[account] = magnifiedDividendCorrections[account].sub(
            (magnifiedDividendPerShare.mul(value)).toInt256()
        );
    }
...
contract ORBNDividendTracker is DividendPayingToken, Ownable {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            'ORBN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours'
        );
...
- burn 2%
*/
    constructor() ERC20('ASVORIA Protocol', 'ASVORIA') {
        ETHRewardFee = 1;
        liquidityFee = 1;
        marketingFee = 1;
        burnFee = 1;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(onSelltotalFees.sub(onSellliquidityFee)).mul(
            onSellmarketingFee
        );
